...
fastq_dir <- "/cluster/projects/nn9745k/nico/02_results/03_hundred_lakes_norway/bacteria/AdaptersRemoved"  # fastq directory with the samples we are using
path_out = "/cluster/projects/nn9745k/nico/02_results/03_hundred_lakes_norway/bacteria/fil24016020"
database_dir <- "/cluster/projects/nn9745k/nico/03_databases/silva138"  # folder with databases
...
#### Filter and Trim
# Using configuration number 1
out = filterAndTrim(fns_R1, filt_R1, fns_R2, filt_R2,
	  truncLen=c(240,160), maxN=0, maxEE=c(2,2),
	  truncQ=20, rm.phix=TRUE, compress=TRUE,
	  multithread=TRUE, matchIDs = TRUE)	#Use multithread=TRUE when running on Saga

# To use configuration number 2, uncomment
#out = filterAndTrim(fns_R1, filt_R1, fns_R2, filt_R2,
#	  truncLen=c(245,170), maxN=0, maxEE=c(2,2),
#	  truncQ=10, rm.phix=TRUE, compress=TRUE,
#	  multithread=FALSE, matchIDs = TRUE)	#Use multithread=TRUE when running on Saga

# To use configuration number 3, uncomment
#out = filterAndTrim(fns_R1, filt_R1, fns_R2, filt_R2,
#	  truncLen=c(245,160), maxN=0, maxEE=c(2,2),
#	  truncQ=5, rm.phix=TRUE, compress=TRUE,
#	  multithread=FALSE, matchIDs = TRUE)	#Use multithread=TRUE when running on Saga
...
Biostrings::writeXStringSet(seq_out, file.path(dada2_dir, "ASV_no_taxo.fasta.gz"),
                            compress = TRUE, width = 20000)

## make training data from mothur output
dada2:::makeTaxonomyFasta_Silva(file.path("/cluster/projects/nn9745k/nico/03_databases/silva138/silva.nr_v138.align"), file.path("/cluster/projects/nn9745k/nico/03_databases/silva138/silva.nr_v138.tax"), "/cluster/projects/nn9745k/nico/03_databases/silva138/silva_nr_v138_train_set.fa.gz")

## assign taxonomy
taxa_silva = assignTaxonomy(seqtab.nochim, file.path("/cluster/projects/nn9745k/nico/03_databases/silva138/silva_nr_v138_train_set.fa.gz"), multithread=TRUE, tryRC = TRUE)
